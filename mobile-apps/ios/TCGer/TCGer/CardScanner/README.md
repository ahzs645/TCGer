...
2. **Embedding inference** – `CardEmbeddingEncoder` is a thin wrapper around a Core ML encoder (for example a SimCLR/Barlow Twins tuned MobileNet). It expects a compiled model named `CardEmbeddings.mlmodelc` in the app bundle and returns the raw float vector produced by the model's `embedding` output. Before the resize/centre-crop contract every query crop passes through `QueryColorNormalization` when the game's acceptance policy declares `queryNormalization: grey-world-autocontrast` (Magic): a grey-world white balance then a per-channel 1 % autocontrast, pixel-exact with the Android and trainer implementations. It exists because hand-held crops arrive low-contrast under a room colour cast while the gallery is embedded from clean renders (2026-08-30: correct family first on 79 → 104 of 108 labeled Magic crops); it is per game because the Pokémon encoder, trained toward camera captures, loses under it.
