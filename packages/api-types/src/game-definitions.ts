...
export const collectionIdentityModeSchema = z
  .object({
...
export const gameDefinitionSchema = z
  .object({
    ...
    aliases: z.array(gameIdSchema).max(12).optional(),
    formats: z
      .array(
        z
          .object({
    ...
    presentation: z
      .object({
        ...
        cardBackUrl: z.string().min(1).max(2048).optional(),
      })
      .strict()
      .optional(),
    interfaces: z
      .object({
        ...
        wishlists: z.boolean().default(true),
        decks: z.boolean().default(false),
        pricing: z.boolean().default(false),
        sealedProducts: z.boolean().default(false),
        scanner: z.boolean().default(false),
        packOpening: z.boolean().default(false),
        features: z.array(gameFeatureSchema).max(32).default([]),
      })
      .strict()
      .optional(),
    collection: z
  ...
  .superRefine((definition, context) => {
    const identityIds = definition.collection.identityModes.map(
      (mode) => mode.id,
    );
    if (new Set(identityIds).size !== identityIds.length) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["collection", "identityModes"],
        message: "Identity mode ids must be unique",
      });
    }
    if (!identityIds.includes(definition.collection.defaultIdentityMode)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["collection", "defaultIdentityMode"],
        message: "Default identity mode must be declared",
      });
    }
    definition.collection.identityModes.forEach((mode, index) => {
      const expectedKey =
        mode.id === "consolidated" ? "baseExternalId" : "printingKey";
      if (mode.key !== expectedKey) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["collection", "identityModes", index, "key"],
          message: `${mode.id} identity must use ${expectedKey}`,
        });
      }
    });
    const featureIds =
      definition.interfaces?.features.map((feature) => feature.id) ?? [];
    if (new Set(featureIds).size !== featureIds.length) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["interfaces", "features"],
        message: "Feature ids must be unique within an interface",
      });
    }
    for (const [surface, facets] of [
      ["collection", definition.collection.facets],
      ["search", definition.search.facets],
    ] as const) {
      ...
      if (new Set(ids).size !== ids.length) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: [surface, "facets"],
          message: "Facet ids must be unique within an interface",
        });
      }
      facets.forEach((facet, index) => {
        if (facet.type === "numberRange" && facet.min > facet.max) {
          context.addIssue({
            code: z.ZodIssueCode.custom,
            path: [surface, "facets", index, "min"],
            message: "Facet min must not exceed max",
          });
        }
      });
    }
...
export type CollectionIdentityMode = z.infer<
  typeof collectionIdentityModeSchema
>;
export type GameFeature = z.infer<typeof gameFeatureSchema>;
export type GameDefinition = z.infer<typeof gameDefinitionSchema>;

export const POKEDEX_GAME_FEATURE_ID = "pokedex";

export interface StandardGameFeatureAdapter {
  id: string;
  version: number;
  label: string;
  requiredCardFields: readonly string[];
}

/**
 * Feature ids without a publisher prefix are reserved for adapters implemented
 * consistently by TCGer clients. Third-party-only adapters use
 * `<publisher-id>--<feature-id>` so two publishers cannot accidentally claim
 * the same UI contract.
 */
export const STANDARD_GAME_FEATURE_ADAPTERS: readonly StandardGameFeatureAdapter[] =
  Object.freeze([
    {
      id: POKEDEX_GAME_FEATURE_ID,
      version: 1,
      label: "Pokédex",
      requiredCardFields: ["dexEntries.number"],
    },
  ]);

export function standardGameFeatureAdapter(
  featureId: string,
): StandardGameFeatureAdapter | undefined {
  return STANDARD_GAME_FEATURE_ADAPTERS.find(
    (adapter) => adapter.id === featureId,
  );
}

export function publisherGameFeatureId(
  publisherId: string,
  featureId: string,
): string {
  return `${publisherId}--${featureId}`;
}

export function gameDefinitionSupportsFeature(
  definition: GameDefinition,
  featureId: string,
  supportedVersion = 1,
): boolean {
  return (
    definition.interfaces?.features.some(
      (feature) =>
        feature.id === featureId && feature.version <= supportedVersion,
    ) ?? false
  );
}
...
const physicalFacets: CollectionFacet[] = [
  {
    id: "language",
    label: "Language",
    property: "copies.language",
    type: "multiSelect",
  },
  {
    id: "edition",
    label: "Edition",
    property: "copies.edition",
    type: "multiSelect",
  },
  {
    id: "owned-quantity",
...
const definitions: Record<TcgCode, GameDefinition> = {
    ...
    search: {
      facets: [
        ...
        },
        {
          id: "rules",
          label: "Rules text",
          property: "attributes.oracle_text",
          type: "text",
          mode: "contains",
          maxLength: 160,
        },
        ...
        },
        {
          id: "reserved",
          label: "Reserved List",
          property: "attributes.reserved",
          type: "boolean",
        },
        {
          id: "full-art",
          label: "Full art",
          property: "attributes.full_art",
          type: "boolean",
        },
      ],
    },
  ...
  pokemon: {
    ...
    interfaces: {
      ...
      scanner: false,
      packOpening: false,
      features: [{ id: POKEDEX_GAME_FEATURE_ID, version: 1 }],
    },
    search: {
      facets: [
        ...commonSearchFacets,
        ...
        },
        {
          id: "types",
          label: "Energy type",
          property: "attributes.types",
          type: "multiSelect",
        },
        {
          id: "hp",
          label: "HP",
          property: "attributes.hp",
          type: "numberRange",
          min: 0,
          max: 1000,
          step: 10,
        ...
        },
        {
          ...
          property: "regulationMark",
          type: "multiSelect",
        },
        {
          id: "standard",
          label: "Standard legal",
          property: "formatLegality.standard",
          type: "boolean",
        },
      ],
    },
    ...
    search: {
      facets: [
        ...
        },
        {
          ...
          property: "attributes.classifications",
          type: "multiSelect",
        },
        {
          id: "cost",
    ...
    search: {
      facets: [
        ...
        },
        {
          id: "era",
          label: "Era",
          property: "attributes.era",
          type: "multiSelect",
        },
        {
          id: "energy",
          label: "Energy",
          property: "attributes.energy",
          type: "numberRange",
          min: 0,
          max: 20,
          step: 1,
...
export const GAME_DEFINITIONS: Readonly<Record<TcgCode, GameDefinition>> =
  definitions;
...
export function getGameDefinitionOrDefault(gameId: string): GameDefinition {
  ...
  return {
    ...
    search: { facets: commonSearchFacets },
    collection: {
      identityModes,
      defaultIdentityMode: "collector",
      facets: physicalFacets,
    },
  };
}
...
function flattenPathValues(value: unknown, keys: string[]): unknown[] {
  if (Array.isArray(value))
    return value.flatMap((item) => flattenPathValues(item, keys));
...
export function collectionFacetValues(
...
export function collectionFacetOptions(
  ...
  cards
    .flatMap((card) => collectionFacetValues(card, facet.property))
    .forEach((value) => {
      if (
        typeof value !== "string" &&
        typeof value !== "number" &&
