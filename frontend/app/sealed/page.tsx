...
export default function SealedPage() {
  ...
  const [deleting, setDeleting] = useState<SealedInventoryResponse | null>(
    null,
  );
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-6 xl:grid-cols-4">
              ...
              {showPricing ? (
                ...
                <StatCard
                  title="Games"
                  value={
                    new Set(inventory.map((item) => item.product.tcg)).size
                  }
...
function AddInventoryDialog({
  ...
  const initialGame =
    selectedGame !== "all" && enabledGames[selectedGame] ? selectedGame : "all";
  ...
  const products = (catalogQuery.data ?? []).filter((product) => {
    ...
    return (
      !needle ||
      `${product.name} ${product.productType} ${product.setCode ?? ""}`
        .toLocaleLowerCase()
        .includes(needle)
    );
  });
      ...
      <DialogContent className="max-w-2xl">
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            if (!selected)
              return setError("Select a product from the catalog.");
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            if (
                              ...
                              return;
                            try {
                              await deleteCustomSealedProduct(
                                token,
                                product.id,
                              );
...
function CustomProductDialog({
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (!input.name || !input.productType)
        throw new Error("Name and product type are required.");
      if (form.msrp.trim() && input.msrp === undefined)
        throw new Error("MSRP must be zero or greater.");
      return product
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>
            {product ? "Edit custom product" : "Create custom product"}
          </DialogTitle>
          <DialogDescription>
            Custom catalog entries are private to your account.
          </DialogDescription>
        </DialogHeader>
        <form
          className="space-y-4"
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="box, tin, bundle…"
                required
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="custom-tcg">Game code</Label>
              <Input
                id="custom-tcg"
                value={form.tcg}
                onChange={set("tcg")}
            ...
            <div className="space-y-2">
              <Label htmlFor="custom-set">Set code</Label>
              <Input
                id="custom-set"
                value={form.setCode}
                onChange={set("setCode")}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="custom-cards">Cards per pack</Label>
              <Input
                id="custom-cards"
                type="number"
                min="1"
                ...
                onChange={set("cardsPerPack")}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="custom-packs">Packs per box</Label>
              <Input
                id="custom-packs"
                type="number"
                min="1"
            ...
            <div className="space-y-2">
              <Label htmlFor="custom-release">Release date</Label>
              <Input
                id="custom-release"
                type="date"
            ...
            <div className="space-y-2">
              <Label htmlFor="custom-msrp">MSRP</Label>
              <Input
                id="custom-msrp"
                type="number"
                min="0"
            ...
            <div className="space-y-2">
              <Label htmlFor="custom-upc">UPC</Label>
              <Input id="custom-upc" value={form.upc} onChange={set("upc")} />
            </div>
            <div className="space-y-2">
...
function EditInventoryDialog({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      updateSealedInventory(token!, item!.id, {
        quantity: Number(quantity),
        purchasePrice: optionalMoney(price),
        purchaseDate: toIsoDate(date),
        notes: notes.trim(),
...
function RecordOpeningDialog({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      createSealedOpening(token!, item!.id, {
    ...
    <Dialog open={!!item} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Opening {item?.product.name} reduces its sealed quantity. Optionally
            link collection copies pulled from it.
          </DialogDescription>
        </DialogHeader>
        <form
          className="space-y-4"
          onSubmit={(event) => {
            ...
            if (
              ...
              return setError(
                `Opened quantity must be between 1 and ${item?.quantity ?? 1}.`,
              );
            mutation.mutate();
          }}
        >
          ...
          <div className="space-y-2">
            ...
            <ScrollArea className="h-36 rounded-md border">
              {collectionsQuery.isLoading ? (
                ...
                <p className="p-3 text-xs text-muted-foreground">
                  Collection copies couldn’t be loaded. You can still record the
                  opening without links.
                </p>
              ) : linkableCopies.length === 0 ? (
                <p className="p-3 text-xs text-muted-foreground">
                  No matching collection copies found.
                </p>
              ) : (
                <div className="divide-y">
                  {linkableCopies.map((copy) => (
                    <label
                      key={copy.id}
                      className="flex cursor-pointer items-center gap-3 p-2.5 text-sm hover:bg-muted/60"
                    >
                      <Checkbox
            ...
            <p className="text-xs text-muted-foreground">
              Copies already linked to another opening will be rejected by the
              server.
            </p>
          </div>
          {error && <p className="text-sm text-destructive">{error}</p>}
          <DialogFooter>
            ...
            <Button type="submit" disabled={mutation.isPending}>
              {mutation.isPending ? "Recording…" : "Record opening"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
    ...
    <Dialog open={!!item} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            This permanently removes {item?.product.name}. Inventory with
            opening history cannot be deleted.
          </DialogDescription>
        </DialogHeader>
        {error && <p className="text-sm text-destructive">{error}</p>}
        <DialogFooter>
...
function RecordSaleDialog({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      recordOpenedCardSale(token!, card!.id, {
      ...
      <DialogContent>
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            if (value === undefined)
              return setError("Enter sale proceeds of zero or greater.");
            mutation.mutate();
          }}
        >
          <div className="space-y-2">
