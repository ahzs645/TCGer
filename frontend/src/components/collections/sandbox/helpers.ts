...
export function conditionScore(value?: string | null) {
  ...
  const index = CONDITION_ORDER.indexOf(
    normalized as (typeof CONDITION_ORDER)[number],
  );
...
export function conditionRangeLabel(copies: { condition?: string | null }[]) {
  ...
  const filtered = copies
    .filter((copy) => copy.condition)
    .map((copy) => copy.condition!);
  if (!filtered.length) {
    return "Not specified";
  }
  const sorted = [...filtered].sort(
    (a, b) => conditionScore(a) - conditionScore(b),
  );
  ...
  const firstLabel =
    CONDITION_COPY[first as keyof typeof CONDITION_COPY]?.label ?? first;
  const lastLabel =
    CONDITION_COPY[last as keyof typeof CONDITION_COPY]?.label ?? last;
