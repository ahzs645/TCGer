...
export function CardScanPanel() {
  ...
  const [scanFilter, setScanFilter] = useState<ScanFilter>(selectedGame === 'all' ? 'all' : selectedGame);
  ...
  useEffect(() => {
    ...
    getCardScanStatsApi(token)
      ...
      .catch((error) => {
        if (cancelled) return;
        setStatsError(error instanceof Error ? error.message : 'Unable to load scan index stats.');
      })
      .finally(() => {
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    const uniqueCandidates = Array.from(
      new Map(result.candidates.map((candidate) => [resultKey(candidate), candidate])).values()
    );
    ...
    <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
      <Card className="overflow-hidden border-dashed">
        <CardHeader>
          ...
          <CardDescription>
            Upload a card photo or open your phone camera. The server compares the image against the cached perceptual hash index.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
              ...
              <div className="flex h-72 flex-col items-center justify-center gap-3 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.9),_rgba(244,244,245,0.6)_55%,_rgba(228,228,231,0.9))] p-6 text-center">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Use a straight-on shot with the full card in frame for the best first-pass match.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="space-y-2">
            ...
            <Select value={scanFilter} onValueChange={(value) => setScanFilter(value as ScanFilter)}>
              ...
              <SelectContent>
                <SelectItem value="all">Auto-detect across all games</SelectItem>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button type="button" variant="outline" onClick={handleClear} disabled={!selectedFile && !result}>
              Reset
            </Button>
          </div>
          ...
          <div className="space-y-3 rounded-xl border bg-muted/30 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Current server-side scan corpus</p>
              </div>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="border-b">
          ...
          <CardDescription>
            Best match first, followed by alternate candidates ranked by perceptual-hash distance.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6 p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="max-w-xl text-sm text-muted-foreground">
                    Upload a phone photo, screenshot, or cropped card image and the matcher will compare it against the local hash map.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex min-h-[420px] items-center justify-center rounded-xl border border-dashed bg-muted/20 p-8 text-center">
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Preprocessing the upload, hashing it, and scoring it against the cached index.
                  </p>
                </div>
              </div>
            </div>
          ...
          {!isScanning && result && bestMatch && (
            <section className="space-y-4">
              <div className="flex flex-wrap items-center gap-2">
                <Badge className={cn('border', confidenceTone(bestMatch.confidence))}>
              ...
              <div className="grid gap-4 rounded-2xl border bg-muted/20 p-4 lg:grid-cols-[220px_1fr]">
                <div className="overflow-hidden rounded-xl border bg-background">
                  <img
                    src={bestMatchCard?.imageUrlSmall ?? bestMatchCard?.imageUrl ?? getCardBackImage(bestMatch.tcg)}
                    alt={bestMatch.name}
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Best Match</p>
                    <h2 className="text-2xl font-heading font-semibold">{bestMatchCard?.name ?? bestMatch.name}</h2>
                    <p className="text-sm text-muted-foreground">
                      {bestMatchCard?.setName ?? bestMatch.setName ?? bestMatch.setCode ?? 'Set unknown'}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm">
                    The current upload did not land under the confidence threshold. Try a tighter crop, flatter angle, or choose a specific game before rescanning.
                  </p>
                </div>
              </div>
            </div>
          ...
          {!isScanning && otherCandidates.length > 0 && (
            <section className="space-y-3">
              <div>
                <h3 className="text-lg font-heading font-semibold">Alternate Candidates</h3>
                <p className="text-sm text-muted-foreground">Useful when the top match is close but not definitive.</p>
              </div>
              <div className="grid gap-4 lg:grid-cols-2">
                {otherCandidates.map((candidate) => {
                  const candidateCard = resolvedCards[resultKey(candidate)];
                  const imageSrc = candidateCard?.imageUrlSmall ?? candidateCard?.imageUrl ?? getCardBackImage(candidate.tcg);

                  return (
                    <div key={resultKey(candidate)} className="flex gap-4 rounded-xl border bg-background p-3">
                      ...
                      <div className="min-w-0 space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          <Badge variant="outline">{GAME_LABELS[candidate.tcg]}</Badge>
                          <Badge className={cn('border', confidenceTone(candidate.confidence))}>
                            {formatConfidence(candidate.confidence)}
                          </Badge>
                        </div>
                        <div>
                          <p className="truncate font-semibold">{candidateCard?.name ?? candidate.name}</p>
                          <p className="text-sm text-muted-foreground">
                            {candidateCard?.setName ?? candidate.setName ?? candidate.setCode ?? 'Set unknown'}
                          </p>
                        </div>
                        <p className="text-xs text-muted-foreground">Distance {candidate.distance}</p>
                      </div>
                    </div>
                  );
                })}
