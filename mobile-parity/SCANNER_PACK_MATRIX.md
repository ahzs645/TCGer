...
|---|---|---|---|
| Capture | Manual camera, single photo, bulk photos, automatic live consensus, and binder pages | Camera/photo capture, automatic two-frame consensus, YOLO/grid binder review, corner correction, and saved page photos implemented; hardware runtime verification pending | Manual, single photo, typed sequential bulk import, and two-frame automatic consensus implemented; binder pages include automatic page-boundary seeding plus an editable guided 3x3 workflow |
| Games | Pokémon, Yu-Gi-Oh!, and Magic | Implemented for classic pHash; the local embedding index is Pokémon-only | Implemented |
| Engines | Automatic-with-fallback, on-device-only, server pHash, and server embedding | On-device, server pHash and selectable server embedding implemented; automatic engine fallback remains partial | All four are selectable and wired; Android Automatic and iOS both select a local embedding path first, and embedding is limited to Pokémon |
...
| Session options | Language, torch, automatic result opening, price mode, and shared web session | Shared sessions and language defaults retained; capability-gated torch, auto-open, authenticated session prices and separate currency totals implemented | Torch, stable-consensus auto-open, authenticated per-card prices/running totals, and shared-session uploads are implemented; language is persisted and attached to debug/shared-session records, while collector-aware recognition remains partial |
| Results | Session tray/review, correction, bulk add, and manual corner adjustment | Select all/none, row selection/removal, clear-uncommitted, selected bulk-add, and immutable committed rows are implemented; four-corner perspective crop correction implemented | Persistent tray, select/remove/clear review, bulk binder add, four-corner adjustment, perspective crop, and production-handler retry implemented |
| Binder pages | Multi-card detection, per-page review/destinations, saved page photos, replacement on retake | YOLO or configurable grid detection, review/correction, destination binder, saved photos and replacement by page number implemented | Automatic page-boundary detection, editable alignment, nine-pocket extraction, sequential recognition, correction/skip, bulk save, and app-private page-photo replacement are implemented |
...
| `scanner.debug.developerAccess` | Seven-activation unlock, testing-tools toggle, and hide-developer-tools action | Implemented for the new workbench: seven-activation unlock, persisted state and hide/reset control; existing diagnostics remain independently accessible | Implemented with persistent unlock state and reset behavior |
...
| `scanner.debug.recording` | Record/pause frames and save/share/clear an exportable run | Implemented for exact recognition-attempt crops with bounded 500-attempt runs | Implemented with exact input JPEGs, deterministic guide crops, and portable image-embedded archives |
...
| `scanner.debug.sessionManagement` | List, select, export/share, and delete recorded sessions | Implemented for local recorded runs, including list, export, import and delete | Implemented with bounded persistent storage, explicit deletion cleanup, portable export, and a scoped FileProvider Sharesheet |
| `scanner.debug.replay` | Import extracted recordings and produce replay accuracy/timing reports | Implemented: imported crop recordings rerun the selected engine and report labeled accuracy, false positives, misses, mean and p95 | Implemented through the production request handler, with sequential accuracy, false-positive/miss, mean, and p95 reporting; legacy metadata-only recordings cannot replay |
...
| `scanner.debug.performance.*` | Every A/B switch listed below | Preload Models is operational; other engine-specific A/B switches remain planned | Fast Shutter Capture implemented; the other nine remain non-operational/planned |
...
| `warmStart` | Preload Scanner Models | On | Implemented |
...
## September 4 implementation verification

See [the implementation report](../docs/interface-parity-implementation.md) for scope and executed checks. New workflows are implemented in source; camera hardware, image-recognition accuracy and native cross-platform UI parity are not inferred from a successful build. Engine-specific experimental A/B controls above remain explicitly tracked.
