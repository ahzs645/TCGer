...
Status: the alias resolution and fork-leakage fixture from requirement 1 are implemented in the accompanying tooling change. The revised corpus assignment, trainer repair, incumbent evaluations, and remaining training gates are still pending.

1. **Record fork aliases before checking whole-archive separation.** `sourceArchiveId` is already required by the manifest schema and enforced independently by `LEAKAGE_DISJOINT`, alongside `sourceAssetIds`. The gap is canonical mapping: `card-seg-j74w1` and `card-seg-j74w1-q8yst` currently have different archive IDs. The release must carry a `sourceArchiveAliases` table mapping every fork/re-export ID to its canonical archive ID, with explicit self-mappings for canonical IDs. Resolve the record’s original `sourceArchiveId` through this table in `corpus_release.leakage_keys_from_record` before disjointness checking; preflight must fail on any unmapped archive ID. Keep the existing independent source-asset check. The new release must also record its computed archive-to-split assignment. Preserve augmentation-family grouping across exports. Keep all of TCGX and the existing binder evaluation sessions held out; accepted-fit counts above are inventory, not final train/validation eligibility counts.
...
- [Release preflight](../../tools/card-geometry/preflight.py): `LEAKAGE_DISJOINT` currently checks declared archive and asset keys; the accompanying change adds required, hash-covered alias mapping and rejects unmapped IDs; the new corpus’s computed assignment remains pending.
...
## Alias tooling follow-up

The release manifest now requires `sourceArchiveAliases`. Its entries are direct mappings to canonical IDs that map to themselves; missing mappings, chains, and cycles fail preflight. Manifest leakage keys contain the resolved canonical ID, while records retain their original archive ID for provenance. Preflight derives canonical keys from records for both consistency and disjointness, so stale or forged manifest keys cannot hide a fork crossing splits. The alias table is included in `corpusHash`.

The deterministic `invalid-fork-archive-leakage` fixture places `card-seg-j74w1` in validation and its `card-seg-j74w1-q8yst` fork in test, with distinct image, session, and instance identities. It must fail exactly `LEAKAGE_DISJOINT`. Additional checks cover unmapped IDs, malformed alias relationships, same-split forks, and manifest tampering.

Validation: all 176 top-level card-geometry tests pass with `jsonschema==4.23.0`, `numpy==2.1.3`, `Pillow==11.1.0`, and `opencv-python-headless==4.10.0.84`; the four compositor tests also pass. The fixture bytes match their deterministic generator, and the audit's relative code links resolve.

Existing immutable release bytes are not rewritten. Releases without the required table fail the updated manifest schema; publish a new release with reviewed aliases and a new corpus hash to use this preflight. Historical releases and results remain attributable to their original tooling revision. Execution order is unchanged: trainer repair and self-validation, incumbents on `real-geometry-evaluation-v6-full`, then corpus and `training-minimums-v3` frozen before any round-two result.
