...
| `hubSimilarity` / `hubDistinctNames` / `hubTopK` | **Hub rejection.** Abstain when at least `hubDistinctNames` *different* card names sit at or above `hubSimilarity` among the top `hubTopK` neighbours (defaults 0.90 / 3 / 5; `hubDistinctNames: 0` disables). A genuine match never looks like that; a degenerate crop — blank, glare-saturated, mis-rectified — lands near many unrelated rows at once (the 0.99-to-a-back-face failure). Checked before any threshold or OCR rescue. Measured 2026-08-29: 0 hits on 364 verified real-camera crops, 8/12 on the degenerate Stone Quarry attempts. Shipped at 2 distinct names since 2026-08-30: on 160 labeled genuine crops (both games, colour-normalized) no correct crop ever shows two different names ≥ 0.90, while degenerate box crops do (Plains/Forest 0.99). |
| `queryNormalization` | Colour normalization of every query crop before the encoder's resize contract: `none` or `grey-world-autocontrast` (grey-world white balance, then per-channel 1 % autocontrast, Pillow semantics, pixel-exact on iOS/Android/trainer). Per game because it depends on the encoder's training domain: the catalog-only Magic encoder ranks the correct family first on 79 → 104 of 108 labeled crops; the Pokémon encoder, trained toward camera captures, loses (76-label replay 53 → 49), so it stays `none`. `SCANNER_QUERY_NORMALIZATION=0/1` overrides for A/B replays on iOS. |
| *(structural)* orientation contradiction | Both orientations of one crop are always evaluated. When each would be accepted on visual evidence (≥ `strongAcceptanceScore`) as a *different* card — unambiguously, i.e. neither orientation's shortlist carries the other's name within `ambiguityMargin` (a basic Water Energy reads Darkness 0.78 / Water 0.77 upside down and keeps its upright accept) — the crop is degenerate and neither answers (`orientationContradiction` in evidence). A card is not two cards depending on which way up it is; on 125 correct accepts across both games the twin never reached a different name above 0.66, while degenerate box crops score Island 0.94 / Plains 0.85. No parameter of its own. |
