...
**Current conclusion:** the experiments and diagnosis are complete, but the candidate models are not ready for promotion. A confirmed importer defect trained on slabs and card subregions as if they were whole cards. **Update, later on 2026-09-06: that import boundary is now repaired, fixture-tested and gated by a new `TARGET_SEMANTICS` preflight check; the repaired material has been recounted, a versioned polygon fit recovered 1,252 real corner instances, and a successor training candidate exists locally under amended policy v4, and on 2026-09-07 that corpus was published, its four measurement configs were frozen and submitted as Hugging Face Jobs (see the [launch record](benchmarks/2026-09-07-successor-bakeoff/README.md)); results are recorded in [RESULTS.md](benchmarks/2026-09-07-successor-bakeoff/RESULTS.md): the category repair halved stray outlines and tightened single-card geometry for the YOLO detectors, YOLOX improved on every slice, binder pages did not improve.** Read the [category repair record](benchmarks/2026-09-06-category-repair/README.md) and the [successor record](benchmarks/2026-09-06-successor-corpus-prep/README.md) first. Do not restart the same old run, claim a frozen successor exists, or ask the user to relabel hundreds of evaluation frames.
...
| Successor corpus preparation | Versioned `polygon-quad-fit-v2` adapter recovers 1,252 corner instances (train 2,176→2,829, validation 194→793); real multi-card frames sliced by measured layout; policy v4 amended with archive slices; assignment recomputed (equals round two); near-duplicate audit zero pairs; human corner-label queue (2,070 train targets) and ingestion sidecar. | [Successor record](benchmarks/2026-09-06-successor-corpus-prep/README.md). Local `card-geometry-training-successor-candidate-v1` combined under v4. **Not published, no frozen experiment config, not trained.** |
| Successor bake-off launch (2026-09-07) | Published `card-geometry-training-successor-candidate-v1` (dataset rev `46049ce8…`), froze four configs under policy v4 with round-two fairness plus declared YOLOX loss repair and evaluation contract v2, published inputs (model commit `10904992…`), submitted four L4 jobs via the CLI. | [Launch record](benchmarks/2026-09-07-successor-bakeoff/README.md). First submission failed at startup (CLI option parsing, exit 126, no inputs touched); resubmitted jobs `6a9e8129e…` (FastViT), `6a9e8129…` (YOLO11n), `6a9e812a…` (YOLO11s), `6a9e812b…` (YOLOX). **All four completed and verified; see [RESULTS.md](benchmarks/2026-09-07-successor-bakeoff/RESULTS.md).** YOLO11s extras 288→177 and recall .90 .459→.554; YOLO11n extras 403→222; YOLOX real recall .50 .601→.646 with 9 extras; binder pages unchanged-to-worse (3 images, boxes-only supervision); FastViT collapsed at .90 through early best-val checkpoint selection on an overfitting run. No promotion. |
...
The import boundary is repaired and gated (see the [category repair record](benchmarks/2026-09-06-category-repair/README.md)); path-forward step 1 is done and step 3's adapter and label-crop diagnostics are done. Step 2 and the local part of step 4 are done in the [successor record](benchmarks/2026-09-06-successor-corpus-prep/README.md): existing labels cannot supply binder/tabletop corners (all box-only), the v2 fit adapter recovered 1,252 slab/stretched-export corner instances, real multi-card frames carry layout slices, and `card-geometry-training-successor-candidate-v1` is combined locally under amended v4 with zero near-duplicate pairs. Steps (b) through (d) were executed on 2026-09-07: see the [launch record](benchmarks/2026-09-07-successor-bakeoff/README.md) for the four submitted jobs. Results are verified and recorded. A local labeling interface now exists for the queued targets (`archive_corner_label_server.py`, instructions in [LABELING.md](benchmarks/2026-09-06-successor-corpus-prep/LABELING.md); journal at `.artifacts/card-geometry/archive-corner-labels/journal.jsonl`, never counts as human evidence until the user has saved frames). Next: collect human corner labels for the 2,070 queued train multi-card targets (binder is the one slice that did not move and cannot move without them), declare a follow-up experiment, and optionally run a v2-fit ablation to attribute the single-card gains. Recognition encoder/index work remains a separate track. The repaired candidate under `.artifacts/card-geometry/releases/real-geometry-category-repair-candidate-v1` is a local smoke build, not a published input; rebuild or publish it from the recorded commands with a receipt. Separately, check the recognition encoder/index findings (wrong-family accept from a human label crop; 53 zero-norm Pokémon index vectors) before interpreting any recognition replay. Keep the old corpus and benchmarks untouched. There is no outstanding reason to wait for the user to compare these same images.
