...
**September 14, 8:10 a.m. PDT — training complete; final comparison recovering locally:** All three 50-epoch jobs completed and their selected checkpoints and individual evaluations were verified. The control selected epoch 40; both quarter-turn runs selected epoch 50. The final Hugging Face CPU audit failed before inference because SciPy was missing. The launch dependency is fixed, and the same four checkpoints are now being evaluated locally through the frozen common CPU pipeline, without retraining or a new paid job. The first two models have passed 100/600 real photos each. No final paired result or production promotion is claimed yet. [Live status](http://127.0.0.1:8773/rotation/) · [Experiment evidence](benchmarks/2026-09-13-rotation-augmentation/README.md). Earlier updates below are history.

**September 14 — durable data preserved:** All three rotation-pilot training jobs completed, and their selected checkpoints were collected. The final common CPU audit failed and still needs recovery; no replacement has been promoted. Storage cleanup removed 9.53 GB of generated local artifacts. A separate, verified 7.91 GB data archive now preserves all 32 complete local geometry release versions, models/indexes and experiment records in `Reference/TCGer-Geometry-Archive/2026-09-14`. A restore test passed for the full 600-photo / 700-card real benchmark. Original working files remain in place, and label backups continue automatically. [Preservation and restore details](benchmarks/2026-09-14-reference-preservation/README.md) · [Cleanup record](benchmarks/2026-09-14-storage-cleanup/README.md). Older status updates below are dated history.

**September 13, 9:39 p.m. PDT — three-run rotation pilot submitted:** The checkpoint-selection backtest is finished. The declared real-weighted selector chose epoch 50 twice: it picked the cyclic run's best tight count, but fixed epoch 30 had 559 tight borders versus 544 at selected epoch 50. Keep this as a provisional selection rule; it is not proven to choose the best benchmark checkpoint. One added fixed/no-augmentation seed and two exact-quarter-turn seeds are submitted on Hugging Face; no flips or larger model. A common CPU comparison will follow, with two seeds per arm and predeclared thresholds. The identity review is ready for 106 card appearances, including 23 unverified phone suggestions, with decisions saved directly to Drive. No new identities have been declared verified and no model has been promoted. [Experiment, jobs and evidence](benchmarks/2026-09-13-rotation-augmentation/README.md) · [Live status](http://127.0.0.1:8773/rotation/) · [Identity review](http://127.0.0.1:8776/). Older updates below are history.

**September 13, 7:30 p.m. PDT — larger benchmark audit complete:** All seven saved YOLO11s checkpoints finished **600 real photos / 700 cards**, **1,000 synthetic photos / 3,651 cards**, and the same 57-frame recognition replay. Seven receipts and 56 output hashes were verified. The final-epoch seed difference in found rate is smaller than on the phone session (+1.43 percentage points real; −0.05 synthetic; +8.49 phone), but one pair still does not measure variance. The September 10 fixed/cyclic models are nearly tied on real data; cyclic leads slightly on synthetic. This does **not** confirm loss as the cause of the phone regression. Framework checkpoint selection still trades border accuracy for detections, and recognition has too few verified identities to select a replacement. Keep the current baseline; defer YOLO11m and define checkpoint selection and the two-seed augmentation experiment before more training. No further full label review is needed. [Results, interpretation and verification](benchmarks/2026-09-13-large-benchmark-audit/README.md). The earlier updates below are dated history.

**September 13 update — phone comparisons complete:** [Both loss/split/seed experiments and phone comparisons have finished](benchmarks/2026-09-13-loss-split-ablation/README.md), superseding the experiment order below. Fixed order on the September 10 split found **91 cards / 31 tight outlines**; the September 8 recipe with a new seed found **98 / 29** at its selected checkpoint. Keep the existing model as the continuity default; its **96 / 40** is an observation on this reused session, not a fixed promotion bar or established general superiority. The final-epoch seed repeat shifted found cards from **89 to 98**; one repeat still does not estimate seed variance. A local evaluation of the latest checkpoints and historical parents on the frozen 700-card real and 3,651-card synthetic benchmarks, including four-way recognition replay, is now running. Orientation replay produced 354 unverified top proposals, all in test; none were applied. YOLO11m remains deferred. [Results and galleries](http://127.0.0.1:8773/ablation/). The remainder of this document preserves the September 11 snapshot.
